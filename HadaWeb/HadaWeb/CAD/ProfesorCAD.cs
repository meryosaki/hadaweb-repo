...
namespace PracticaGrupalHADA
{
    class ProfesorCAD
    {
        ...
        public ProfesorEN mostrar_profesor(int id)
        {
            SqlDataReader dr;
            ProfesorEN usuario = new ProfesorEN();
            try
            {
                conex.Open();
                ...
                dr.Read();
                usuario.IdUsuario = Int32.Parse(dr["idMensaje"].ToString());
                usuario.Email = dr["email"].ToString();
                usuario.Nick = dr["nick"].ToString();
                usuario.Nombre = dr["nombre"].ToString();
                usuario.Contrasenya = dr["contraseña"].ToString();
